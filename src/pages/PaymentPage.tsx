...
// TODO: Integrate Razorpay server-side order creation and webhook verification
...
const PaymentPage = () => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // Step 2: Simulate Razorpay checkout (mock)
      // TODO: Replace with real Razorpay.open() call
      const mockPaymentId = `pay_${Date.now()}`;
      const mockSignature = `valid_${orderId}`;

      setProcessing(true);

      // Step 3: Send webhook to activate subscription
  ...
  if (loading) {
    return (
      <Layout>
        <div className="container py-8 px-4 max-w-lg mx-auto space-y-4">
